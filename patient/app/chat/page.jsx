...
const ChatPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const currentUser = await fetch("http://localhost:4000/user/me", {
        method: "GET",
        credentials: "include",
      });
      if (!currentUser) {
      ...
      const dbguser = await currentUser.json();
      console.log("VALUE OF THE CURRENT USER: ", dbguser);
      setUser(dbguser);
      ...
      // Fetch user's chats
      await fetchUserChats(dbguser.user.id);
  ...
  useEffect(() => {
    ...
    socket.on("typing_start", (data) => {
      if (data.chat_id === currentChat.chat_id && data.user_type === "doctor") {
        setTyping(true);
      }
    });
  ...
  const fetchUserChats = async (userId) => {
    try {
      ...
      console.log("Data dot chats: ", data.chats);
      setChats(data.chats);
              ...
              {chats.map((chat) => (
                ...
                >
                  <Avatar className="h-12 w-12 mr-4">
                    <AvatarImage src={chat.doctor_avatar || ""} alt="Doctor" />
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="font-medium">
                    Dr. {currentChat.doctor_name || "Doctor"}
                  </h3>
                  {typing && (
